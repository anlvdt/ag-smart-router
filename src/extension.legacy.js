...
function loadWiki() {
    ...
    if (saved && saved.index) {
        _wiki = saved;
        // Migrate: ensure newer fields exist on old wiki data
        ...
        // Reset transient state — _lastCmd is session-only, stale timestamps cause false sequence links
...
function wikiIngest(cmd, action, data, context) {
    ...
    if (context.project) {
        if (!page.tags.includes('proj:' + context.project)) {
            ...
            // Prune project tags to prevent unbounded growth (keep last 10 projects)
    ...
    if (_wiki._lastCmd.cmd && _wiki._lastCmd.cmd !== cmd && (now - _wiki._lastCmd.time) < 30000) {
        ...
        // Prune sequences to prevent unbounded growth (keep top 500 by count)
        ...
        if (seqKeys.length > 500) {
            ...
            _wiki.sequences = {};
            for (let si = 0; si < 300 && si < sorted.length; si++) {
                _wiki.sequences[sorted[si][0]] = sorted[si][1];
            }
        }
    }
...
function wikiLint() {
    ...
    // Clean up concept.commands — remove commands no longer in wiki index
    for (const [, cp] of Object.entries(_wiki.concepts)) {
